...
void set_mac_to_fuses(int dev_id)
{
	struct ocotp_regs *ocotp = (struct ocotp_regs *)OCOTP_BASE_ADDR;
		...
		if (0 == dev_id) {
			writel(0x00000004, &fuse->mac_addr1);
			writel(0x9F042675, &fuse->mac_addr0);
		}
		else{
			writel(0x00049F04, &fuse->mac_addr2);
			writel(0x26750000, &fuse->mac_addr1);
		}
...
#if defined(CONFIG_VIDEX)
	set_mac_to_fuses(dev_id); //videx changes
#endif
